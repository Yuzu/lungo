...
import ShieldController from "../Homework5/Player/ShieldController";
...
export default class Platformer extends Scene {
    ...
    loadScene(){
        ...
        //Load the shield spritesheet
        this.load.spritesheet("shield", "demo_assets/spritesheets/platformer/shield.json");
    ...
    startScene(){
        ...
        // Add the shield in the starting position
        this.shield = this.add.animatedSprite("shield", "primary");
        this.shield.animation.play("IDLE");
        this.shield.position.set(3*16, 18*16);

        // Add physics so the player can move
        this.player.addPhysics();
        this.player.addAI(PlayerController, {jumpSoundKey: "jump"});

        // Add physics so the shield can move
        this.shield.addPhysics();
        this.shield.addAI(ShieldController);
